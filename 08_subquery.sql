-- 메뉴 테이블에서 민트 미역국과 같은 카테고리 코드를 가지고 있는 메뉴조회
...
-- 상관 서브쿼리
-- 메인쿼리문이 서브쿼리의 결과에 영향을 주는 경우를 상관 서브쿼리 라고한다.
...
-- from 절에서만 사용이 된다.(join 일 시 join 구문에서도 사용가능)

with menucte as (select menu_name,category_name
